...
import { createAuditLogService, IAuditLogService, AuditLogFilter } from '../audit/index.js';
...
export function createAuditRoutes(): Router {
  ...
  router.get('/audit-log/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, entryId: req.params.id }, 'Error fetching audit log entry');
      res.status(500).json({ 
        error: 'Internal server error' 
      });
    }
  ...
  router.post('/audit-log/:id/approve', async (req: Request, res: Response) => {
    try {
      ...
      // Update the entry
      const updatedEntry = await service.updateEntry(id, 'APPROVED', decisionBy);
      if (!updatedEntry) {
      ...
      logger.error({ error, entryId: req.params.id }, 'Error approving audit log entry');
      res.status(500).json({ 
        error: 'Internal server error' 
      });
    }
  ...
  router.get('/audit-log/status', async (req: Request, res: Response) => {
    try {
      const status = {
        ...
        approvalTimeoutMs: parseInt(process.env.CCO_APPROVAL_TIMEOUT || '300000')
      };
  ...
  router.post('/audit-log/:id/deny', async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, entryId: req.params.id }, 'Error denying audit log entry');
      res.status(500).json({ 
        error: 'Internal server error' 
      });
    }
