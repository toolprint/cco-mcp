...
const APPROVAL_TIMEOUT_MS = parseInt(process.env.CCO_APPROVAL_TIMEOUT || '300000'); // 5 minutes default
...
server.tool(
  ...
  "Request approval for a tool call - integrates with audit log",
  {
    ...
    agent_identity: z.string().optional().describe("Identity of the agent making the request"),
  },
  async ({ tool_name, input, agent_identity }) => {
    ...
    try {
      ...
      const entry = await auditService.addEntry(tool_name, input, agent_identity);
      logger.info({ entryId: entry.id }, "Created audit log entry for approval request");
