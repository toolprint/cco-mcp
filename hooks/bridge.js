  ...
  try {
    ...
    if (eventData.type === 'PreToolUse') {
      ...
      if (!blockingResponse || typeof blockingResponse !== 'object') {
        logWarn('Invalid blocking response, defaulting to allow', { response: blockingResponse });
        return {
          behavior: 'allow',
      ...
      if (!blockingResponse.behavior || !['allow', 'deny'].includes(blockingResponse.behavior)) {
  ...
  process.stdin.on('data', async (chunk) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (trimmedLine.includes('"type":"PreToolUse"') || trimmedLine.includes("'type':'PreToolUse'")) {
