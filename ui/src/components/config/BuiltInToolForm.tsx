...
export const BuiltInToolForm: React.FC<BuiltInToolFormProps> = ({
  ...
  const selectedTool = toolName in BUILT_IN_TOOLS ? BUILT_IN_TOOLS[toolName as BuiltInToolName] : null;
  const showSpecifierWarning = optionalSpecifier === "*" || optionalSpecifier === "(*)" || 
    ...
    <div className="space-y-4">
      ...
      {selectedTool && selectedTool.requiresPermission && (
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
            Optional Specifier
            <span className="text-xs text-gray-500 ml-2">(Leave empty for no restrictions)</span>
          </label>
          <Input
            type="text"
            value={optionalSpecifier}
            onChange={(e) => onSpecifierChange(e.target.value)}
            placeholder={selectedTool.exampleSpecifiers?.[0] || "Enter specifier..."}
            className="w-full"
          />
          
          {/* Examples */}
          {selectedTool.exampleSpecifiers && selectedTool.exampleSpecifiers.length > 0 && (
          ...
          {showSpecifierWarning && (
            <div className="flex items-start gap-2 mt-2 p-3 bg-orange-50 dark:bg-orange-950/20 rounded-md border border-orange-200 dark:border-orange-800">
              ...
              <p className="text-xs text-orange-800 dark:text-orange-200">
                <span className="font-medium">Security Notice:</span> Using <code className="font-mono">(*)</code> or 
                leaving empty allows unrestricted use of this tool. Consider being more specific to limit permissions.
              </p>
            </div>
          )}
        </div>
      ...
      {selectedTool && toolName === 'Bash' && (
        <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
          <strong>Note:</strong> Claude is aware of shell operators. A prefix rule like <code>npm run test:*</code> 
          won't allow chained commands like <code>npm run test:unit && rm -rf /</code>.
        </div>
      )}

      {selectedTool && (toolName === 'Edit' || toolName === 'Read' || toolName === 'Write' || toolName === 'MultiEdit') && (
        <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
          <strong>Note:</strong> File patterns follow gitignore specification. Use <code>**</code> for recursive matching.
        </div>
      )}

      {selectedTool && toolName === 'WebFetch' && (
        <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md">
          <strong>Note:</strong> Use <code>domain:</code> prefix to restrict by domain, e.g., <code>domain:example.com</code>.
        </div>
      )}
    </div>
  );
