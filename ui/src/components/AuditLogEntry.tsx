...
import { Clock, User, Check, X, AlertCircle, ChevronRight, ChevronDown, Terminal, Server, UserCircle } from 'lucide-react';
...
export const AuditLogEntry: React.FC<AuditLogEntryProps> = ({ 
  ...
  return (
    <Card className={cn(
      ...
      entry.state === 'NEEDS_REVIEW' && !isExpired && 'border-blueprint-400 shadow-blueprint-100',
      ...
      <CardContent className="p-0">
              ...
              <div className="flex-1">
                ...
                {entry.state !== 'NEEDS_REVIEW' && (
                  <div className="mt-2 flex items-center gap-2 text-sm">
                    {entry.state === 'APPROVED' && entry.decision_by && (() => {
                      const identity = formatUserIdentity(entry.decision_by);
                      return (
                        <div className="flex items-center gap-2">
                          <span className="text-status-success">Approved by</span>
                          <Badge 
                            variant="outline" 
                            className="text-xs border-status-success/30 text-status-success"
                          >
                            {identity.type === 'ip' && <Server className="h-3 w-3 mr-1" />}
                            {identity.type === 'user' && <UserCircle className="h-3 w-3 mr-1" />}
                            {identity.display}
                          </Badge>
                        </div>
                      );
                    ...
                    {entry.state === 'DENIED' && entry.denied_by_timeout && (
                      <div className="flex items-center gap-2">
                        <span className="text-status-danger">Denied by timeout</span>
                        <Badge 
                          variant="outline" 
                        ...
                        >
                          ...
                          {entry.expires_at ? 
                            formatDuration(new Date(entry.expires_at).getTime() - new Date(entry.timestamp).getTime()) : 
                            'expired'}
                        </Badge>
                      </div>
                    )}
                    {entry.state === 'DENIED' && !entry.denied_by_timeout && entry.decision_by && (() => {
                      const identity = formatUserIdentity(entry.decision_by);
                      return (
                        <div className="flex items-center gap-2">
                          <span className="text-status-danger">Denied by</span>
                          <Badge 
                            variant="outline" 
                            className="text-xs border-status-danger/30 text-status-danger"
                          >
          ...
          <div className="px-4 pb-4 space-y-3">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Request Details</p>
                <JsonViewer 
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                    {entry.state === 'APPROVED' ? 'Response' : 'Response Preview'}
                  </p>
        ...
        {entry.state === 'NEEDS_REVIEW' && !isExpired && onApprove && onDeny && (
