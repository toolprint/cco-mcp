        ...
        <div>
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              onFiltersChange({
                ...filters,
                type: value === "" ? undefined : (value as HookEventType),
              });
            }}
          >
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              onFiltersChange({
                ...filters,
                agentIdentity: e.target.value || undefined,
              })
            }
          />
        </div>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3 flex-1">
          <span className="text-2xl">
            {getEventIcon(event.type as HookEventType)}
          </span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <Badge className={EVENT_TYPE_COLORS[event.type as HookEventType]}>
                {event.type}
              </Badge>
              {event.evaluation && (
            ...
            <div className="space-y-1 text-sm">
              <div>
                ...
                <code className="text-xs">{event.sessionId}</code>
              </div>
              ...
              {event.stop_hook_active !== undefined && (
                <div>
                  <strong>Stop Hook Active:</strong>{" "}
                  {event.stop_hook_active ? "Yes" : "No"}
                </div>
              )}

              {event.agentIdentity && (
                <div>
                  <strong>Agent:</strong> {event.agentIdentity}
                </div>
              )}
              ...
              {event.transcript_path && (
                <div>
                  ...
                  <code className="text-xs">{event.transcript_path}</code>
                </div>
              )}
              ...
              {event.cwd && (
                <div>
                  ...
                  <code className="text-xs">{event.cwd}</code>
                </div>
              )}

              {event.auditEntryId && (
                <div>
                  <strong>Audit Entry:</strong>{" "}
                  <code className="text-xs">{event.auditEntryId}</code>
                </div>
              )}
              ...
              {event.tool_input && Object.keys(event.tool_input).length > 0 && (
                <div>
                  ...
                  <code className="text-xs">
                    ({Object.keys(event.tool_input).length} fields)
                  </code>
                </div>
              )}
              ...
              {event.evaluation && (
                <div className="mt-2 p-2 bg-gray-50 rounded">
                  <div>
                    <strong>Evaluation:</strong> {event.evaluation.behavior}
                  </div>
                  {event.evaluation.message && (
                    <div>
                      <strong>Message:</strong> {event.evaluation.message}
                    </div>
                  )}
                  {event.evaluation.ruleName && (
                    <div>
                      <strong>Rule:</strong> {event.evaluation.ruleName}
                    </div>
                  )}
                  <div>
                    <strong>Time:</strong> {event.evaluation.evaluationTime}ms
                  </div>
                </div>
              )}
            </div>
      ...
      <Card className="p-4">
        ...
        <p className="text-lg">
          <span className="text-green-600 font-semibold">
            {stats.evaluationStats.allowed}
          </span>
          {" / "}
          <span className="text-red-600 font-semibold">
            {stats.evaluationStats.denied}
          </span>
        </p>
...
export function Events() {
  ...
  const {
    ...
    error: queryError,
    refetch,
  } = useHookEvents(filters);
  const {
    events: sseEvents,
    connected: sseConnected,
    error: sseError,
  } = useHookEventSSE(useRealTime ? filters : undefined);
  ...
  const events =
    useRealTime && sseConnected ? sseEvents : queryData?.events || [];
          ...
          {!loading && events.length === 0 && (
            <Card className="p-8 text-center">
              ...
              <p className="text-gray-600">
                No hook events match your current filters. Try adjusting your
                filters or check that Claude Code hooks are configured.
              </p>
            </Card>
          )}
