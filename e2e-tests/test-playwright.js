  ...
  try {
    ...
    try {
      ...
      await page.screenshot({
        path: `recordings/playwright-test-${timestamp}.png`,
      });
      console.log(
        `✓ Screenshot saved as recordings/playwright-test-${timestamp}.png`
      );
    } catch (error) {
      console.log(
        "⚠ Could not connect to http://localhost:8660 (server may not be running)"
      );
    }
